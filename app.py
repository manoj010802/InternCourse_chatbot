...
class ChatApplication:
    ...
    def setup_main_window(self):
        ...
        self.window.configure(width=480,height=600,bg=BG_COLOR)

        head_label=Label(self.window,bg=BG_COLOR,fg=TEXT_COLOR,text="Welcome",font=FONT_BOLD,pady=10)
        head_label.place(relwidth=1)

        line=Label(self.window,width=450,bg=BG_GRAY)
        line.place(relwidth=1,rely=0.077,relheight=0.012)

        self.text_widget=Text(self.window,width=20,height=2,bg=BG_COLOR,fg=TEXT_COLOR,font=FONT,padx=5,pady=5)
        self.text_widget.place(relheight=0.80,relwidth=1,rely=0.08)
        ...
        send_button.place(relx=0.77,rely=0.000,relheight=0.06,relwidth=0.22)
